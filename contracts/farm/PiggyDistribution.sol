...
contract PiggyDistribution is IPiggyDistribution, Exponential, OwnableUpgradeable {
    ...
    event WpcSpeedUpdated(IPToken indexed pToken, uint newSpeed);

    /// @notice Emitted when WPC is distributed to a supplier
    event DistributedSupplierWpc(IPToken indexed pToken, address indexed supplier, uint wpcDelta, uint wpcSupplyIndex);

    /// @notice Emitted when WPC is distributed to a borrower
    event DistributedBorrowerWpc(IPToken indexed pToken, address indexed borrower, uint wpcDelta, uint wpcBorrowIndex);
    ...
    function distributeBorrowWpc(address pToken, address borrower, bool distributeAll) public override(IPiggyDistribution) {
        ...
        if (enableDistributeBorrowWpc) {
            Exp memory borrowIndex = Exp({mantissa : IPToken(pToken).borrowIndex()});
    ...
    function setWpcSpeedInternal(IPToken pToken, uint wpcSpeed) internal {
    ...
    function updateWpcSupplyIndex(address pToken) internal {
        ...
        if (deltaBlocks > 0 && supplySpeed > 0) {
            uint supplyTokens = IPToken(pToken).totalSupply();
    ...
    function updateWpcBorrowIndex(address pToken, Exp memory marketBorrowIndex) internal {
        ...
        if (deltaBlocks > 0 && borrowSpeed > 0) {
            uint borrowAmount = div_(IPToken(pToken).totalBorrows(), marketBorrowIndex);
    ...
    function distributeSupplierWpc(address pToken, address supplier, bool distributeAll) internal {
        ...
        uint supplierTokens = IPToken(pToken).balanceOf(supplier);
        ...
        emit DistributedSupplierWpc(IPToken(pToken), supplier, supplierDelta, supplyIndex.mantissa);
    }
    ...
    function distributeBorrowerWpc(address pToken, address borrower, Exp memory marketBorrowIndex, bool distributeAll) internal {
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            uint borrowerAmount = div_(IPToken(pToken).borrowBalanceStored(borrower), marketBorrowIndex);
            ...
            emit DistributedBorrowerWpc(IPToken(pToken), borrower, borrowerDelta, borrowIndex.mantissa);
        }
    }
     ...
     */
    function claimWpc(address holder, IPToken[] memory pTokens) public {
     ...
     */
    function claimWpc(address[] memory holders, IPToken[] memory pTokens, bool borrowers, bool suppliers) public {
        require(enableWpcClaim, "Claim is not enabled");

        for (uint i = 0; i < pTokens.length; i++) {
            IPToken pToken = pTokens[i];
    ...
    function _setWpcSpeed(IPToken pToken, uint wpcSpeed) public onlyOwner {
    ...
    function pendingWpcInternal(address holder, bool borrowers, bool suppliers) internal view returns (uint256){
        ...
        IPToken[] memory pTokens = comptroller.getAllMarkets();
        for (uint i = 0; i < pTokens.length; i++) {
    ...
    function pendingWpcBorrowInternal(address borrower, address pToken) internal view returns (uint256){
        if (enableDistributeBorrowWpc && enableDistributeRepayBorrowWpc) {
            Exp memory marketBorrowIndex = Exp({mantissa : IPToken(pToken).borrowIndex()});
